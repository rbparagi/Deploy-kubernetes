...
# Query to retrieve URL, visit time, and visit duration for today for the specific URL
query = f"""
SELECT 
    urls.url, 
    visits.visit_time, 
...
WHERE 
    ...
    visits.visit_time BETWEEN {today_start_timestamp} AND {today_end_timestamp} AND
    urls.url LIKE '%learn.kodekloud.com%'
"""
...
# Calculate total time spent on the specific URL
time_spent = df.groupby('URL')['Visit Duration (minutes)'].sum().sort_values(ascending=False)

# Display the details for the specific URL
print(time_spent)
